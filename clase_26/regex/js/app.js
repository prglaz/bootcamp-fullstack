...
const regexEmail = /^[a-zñ0-9._]+@[a-z0-9-]+.(com$|com.[a-z0-9]{2}$)/
const regexAge = /^([1-9]{1}\d{0,1})$|^100$/

function validate() {
    if (!regexEmail.test(inputEmail.value)) {
        inputEmail.nextElementSibling.innerText = "El email es invalido"
    }
    else {
    ...
    if (!regexAge.test(inputAge.value)) {
        inputAge.nextElementSibling.innerText = "La edad debe estar entre 1 y 100"
    }
    else {
